...
{
  Dicionario[] dados;  // vetor de Dicionárrio
  ...
  public bool ExisteSemOrdem(string PalavraProcurada, out int ondeEsta)  // pesquisa sequencial em vetor desordenado
  {
    ...
    {
      if (dados[ondeEsta].Palavra == PalavraProcurada)
  ...
  public bool ExisteEmOrdem(string codigoProcurado, out int onde)  // pesquisa sequencial em vetor ordenado
  {
    bool fim, achou;
    ...
    while (!achou && !fim) // not (achou or fim) // De Morgan
      if (onde >= qtosDados) // condição i
        ...
        if (dados[onde].Palavra == codigoProcurado)
           achou = true; // condição ii
        else
          if (codigoProcurado.CompareTo(dados[onde].Palavra) < 0)
  ...
  //pesquisa binária
  public bool Existe(string codigoProcurado, out int onde)  // onde --> posicao onde achou ou onde deveria estar (inclusão)
  {
    ...
    {
      onde = (inicio + fim) / 2;
      if (codigoProcurado == dados[onde].Palavra)
